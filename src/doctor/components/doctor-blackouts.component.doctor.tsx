...
/*                               Schemas & Types                              */
/* -------------------------------------------------------------------------- */
const blackoutSlotSchema = z.object({
...
export default function DoctorBlackouts() {
  ...
  const computedValues = useMemo(() => {
    ...
    if (blackoutsList.length > 0) {
      const formattedBlackouts = blackoutsList.map((b) => {
        // Parse UTC ISO directly to keep selected times matching exactly
        const start_date = b.start_at_utc ? b.start_at_utc.split("T")[0] : "";
        const start_time = b.start_at_utc
          ? b.start_at_utc.split("T")[1].substring(0, 5)
          : "";

        const end_date = b.end_at_utc ? b.end_at_utc.split("T")[0] : "";
        const end_time = b.end_at_utc
          ? b.end_at_utc.split("T")[1].substring(0, 5)
          : "";
  ...
  const onSubmit = (values: BlackoutsFormValues) => {
    ...
    // Use manually constructed ISO strings to avoid local timezone shifts
    const payload = newBlackouts.map((b) => {
      const startIso = `${b.start_date}T${b.start_time}:00.000Z`;
      const endIso = `${b.end_date}T${b.end_time}:00.000Z`;
