...
function TimeIntervals({ value, onChange, className }: TimeIntervalsProps) {
  // Derive internal selection from the external 'value' prop
  const selectedTimeIntervals = useMemo(() => {
    ...
    } else if (TIME_INTERVALS.includes(value)) {
      return [value];
    }
    return [];
  }, [value]);
              ...
              <div key={group.title} className="space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2">
                  {group.periods.map((interval) => {
                    ...
                    return (
                      <Tooltip key={interval} delayDuration={300}>
                        <TooltipTrigger asChild>
                          ...
                          >
                            {isSelected && (
                              <motion.div
                                layoutId={
                                  isFirst ? "active-time-slot" : undefined
                                }
                        ...
                        <TooltipContent className="px-3 py-1.5 text-xs">
                          {isSelected
                            ? `Click to adjust duration or clear selection`
                            : `Click to select ${interval} - ${getEndTime(interval)} (15 mins)`}
                        </TooltipContent>
                      </Tooltip>
                    );
                  })}
