...
function ViewSchedules({ data, loading, user, datesSetHandler }: Props) {
  ...
  const handleNavigateToConsultation = useCallback(
    (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
      ...
      navigate(
        `/${proxy}/dashboard/appointments/${selectedEvent?.event._def.extendedProps._id}`,
      );
    },
  ...
  return (
    <div className="relative p-5" ref={calendarRef}>
      <Calendar
        data={data}
      ...
      {selectedEvent && (
        <div
          ref={popoverRef}
          className="absolute z-50 bg-white border border-gray-100 rounded-xl shadow-2xl p-4 text-sm min-w-[300px] max-w-sm"
          style={{
            top: `${offset.Y}px`,
            left: `${offset.X}px`,
          }}
