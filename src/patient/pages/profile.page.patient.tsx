...
export default function PatientProfile() {
  ...
  const removePhotoMutation = useMutation({
    mutationFn: () =>
      updatePatientProfileReq({
        profile_picture_url: "", // ✅ clears the image on the server
      }),
    onSuccess: () => {
        ...
        <div className="px-6 py-4 space-y-8">
          <div className="flex gap-[24px] items-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              {/* ✅ Remove photo — disabled when not editing OR no photo to remove */}
              <Button
                type="button"
                variant="destructive"
                className="w-full sm:w-[292.5px] min-w-[250px] bg-[#FDE7E5] hover:bg-[#FDE7E5]/80 text-[#D92D20] transition-colors"
                onClick={() => {
                  // If there's only a local preview not yet uploaded, just clear it
                  if (pendingFile) {
                    setPreviewImage(null);
                    setPendingFile(null);
                    return;
                ...
                disabled={
                  !isEditing || // ✅ blurred out when not in edit mode
                  !hasPhoto || // ✅ disabled when nothing to remove
